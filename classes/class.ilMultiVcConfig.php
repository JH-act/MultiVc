...
//            'guestlinkChoose',
    ...
    public function getTokenFromDb(int $connId): array
    {
    ...
    public function getAccessTokenFromDb(int $connId): string
    {
    ...
    public function getTokenUser(string $user): string
    {
        $array = json_decode($this->getAccessToken(), 1);
        $token = isset($array[$user]) ? $array[$user] : '';
        return $token;
    }

    public function setTokenUser(string $user, string $token): void
    {
        $tokenUser = json_decode($this->getAccessToken(), 1);
    ...
    public function setAssignedRoles(array|string|null $assignedRoles): void
    {
    ...
    private function getAllLocalRoles(string $a_str = '%'): string
    {
        global $DIC;
